...
const OtpVerification: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const response = await fetch('https://wemeet-backend.onrender.com/otp/verification/'+params.id, {
                method: "POST",
                headers: {
    ...
    const handleResend = async () => {
        const response = await fetch('https://wemeet-backend.onrender.com/resend/otp/'+params.id, {
            method: "GET",
            headers: {
                "Content-Type" : "application/json"
            }
    ...
    return (
        <div className='bg-slate-100 h-[100vh] w-[100vw] flex flex-col gap-5 justify-center items-center'>
            ...
            <div className='bg-white border-2 rounded-xl w-max h-max flex flex-col justify-evenly gap-5 p-5 shadow'>
                ...
                <button onClick={handleSubmit} className='bg-slate-800 p-2 font-medium text-white rounded'>Submit</button>
            </div>
        </div>
    )
}
