                ...
                <div className='w-max h-max flex flex-col justify-evenly gap-5 p-5 shadow'>
                    ...
                    <form method='POST' className='flex flex-col justify-arounf items-center gap-3'>
                        <span className='flex flex-col gap-1'>
                            ...
                            <input className='px-2 h-[2.25rem] w-[65vw] md:w-[35vw] border rounded' type="text" name='name' value={eventContext?.event?.name} onChange={eventContext?.handleChange} />
                        </span>
                        <span className='flex flex-col gap-1'>
                            <p className='text-sm text-gray-600'>Duration - in miunutes</p>
                            <input className='px-2 h-[2.25rem] w-[65vw] md:w-[35vw] border rounded' type="number" name='duration' value={eventContext?.event?.duration} onChange={eventContext?.handleChange} />
                        </span>
                        <span className='flex flex-col gap-1'>
                            <p className='text-sm text-gray-600'>Description</p>
                            <input className='px-2 h-[2.25rem] w-[65vw] md:w-[35vw] border rounded' type="text" name='description' value={eventContext?.event?.description} onChange={eventContext?.handleChange} />
                        </span>
                    </form>
                    <button onClick={(e) => { e.preventDefault(); eventContext?.handleSubmit(e, params.id) }} className='bg-slate-800 p-2 font-medium text-white rounded'>Create Event</button>
                </div>
            </div>
            :
